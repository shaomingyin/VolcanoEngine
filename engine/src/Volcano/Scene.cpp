...
const QList<Object *> &Scene::objects(void) const
{
    return m_objects;
}

QQmlListProperty<Object> Scene::qmlObjects(void)
{
    return { this, this,
        &Scene::appendObject,
        &Scene::objectCount,
...
{
    auto scene = reinterpret_cast<Scene *>(list->data);
    auto backup = std::move(scene->m_objects);
    for (auto o: backup)
        emit scene->objectRemoved(o);
}

void Scene::replaceObject(QQmlListProperty<Object> *list, qsizetype i, Object *object)
{
    auto scene = reinterpret_cast<Scene *>(list->data);
    auto oldObject = scene->m_objects.at(i);
    scene->m_objects.replace(i, object);
    emit scene->objectRemoved(oldObject);
    emit scene->objectAdded(object);
}
...
{
    auto scene = reinterpret_cast<Scene *>(list->data);
    if (!scene->m_objects.isEmpty()) {
        auto backup = scene->m_objects.last();
        scene->m_objects.removeLast();
        emit scene->objectRemoved(backup);
    }
}
