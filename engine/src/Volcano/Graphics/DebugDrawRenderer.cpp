...
const char * DebugDrawRenderer::c_linePointVertShaderSrc = "\n"
    "#version 150\n"
    "\n"
    "in vec3 in_Position;\n"
...
const char * DebugDrawRenderer::c_linePointFragShaderSrc = "\n"
    "#version 150\n"
    "\n"
    "in  vec4 v_Color;\n"
...
const char * DebugDrawRenderer::c_textVertShaderSrc = "\n"
    "#version 150\n"
    "\n"
    "in vec2 in_Position;\n"
...
const char * DebugDrawRenderer::c_textFragShaderSrc = "\n"
    "#version 150\n"
    "\n"
    "in vec2 v_TexCoords;\n"
    "in vec4 v_Color;\n"
...
{
    VOLCANO_ASSERT(points != nullptr);
    VOLCANO_ASSERT(count > 0 && count <= DEBUG_DRAW_VERTEX_BUFFER_SIZE);
...
void DebugDrawRenderer::drawGlyphList(const dd::DrawVertex * glyphs, int count, dd::GlyphTextureHandle glyphTex)
{
    ...
    glUniform2f(m_textProgram_ScreenDimensions,
                static_cast<GLfloat>(m_size.width()),
                static_cast<GLfloat>(m_size.height()));
...
{
    VOLCANO_ASSERT(width > 0 && height > 0);
    VOLCANO_ASSERT(pixels != nullptr);
...
void DebugDrawRenderer::setupShaderPrograms(void)
{
    //
    ...
    //
    {
        ...
        glShaderSource(linePointVS, 1, &c_linePointVertShaderSrc, nullptr);
        compileShader(linePointVS);

        GLint linePointFS = glCreateShader(GL_FRAGMENT_SHADER);
        glShaderSource(linePointFS, 1, &c_linePointFragShaderSrc, nullptr);
        ...
        {
            VOLCANO_LOGE("Unable to get u_MvpMatrix uniform location!");
        }
        checkGLError(__FILE__, __LINE__);
    }
    ...
    //
    {
        ...
        glShaderSource(textVS, 1, &c_textVertShaderSrc, nullptr);
        compileShader(textVS);

        GLint textFS = glCreateShader(GL_FRAGMENT_SHADER);
        glShaderSource(textFS, 1, &c_textFragShaderSrc, nullptr);
        ...
        {
            VOLCANO_LOGE("Unable to get u_glyphTexture uniform location!");
        }

        m_textProgram_ScreenDimensions = glGetUniformLocation(m_textProgram, "u_screenDimensions");
        if (m_textProgram_ScreenDimensions < 0)
        {
            VOLCANO_LOGE("Unable to get u_screenDimensions uniform location!");
        }
...
void DebugDrawRenderer::setupVertexBuffers(void)
{
...
void DebugDrawRenderer::compileShader(const GLuint shader)
{
...
void DebugDrawRenderer::linkProgram(const GLuint program)
{
