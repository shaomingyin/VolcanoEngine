...
template <typename T>
void qmlRegisterType(const char *uri, const char *qmlName)
{
    ::qmlRegisterType<T>(uri, VOLCANO_VERSION_MAJOR, VOLCANO_VERSION_MINOR, qmlName);
}

template <typename T>
void qmlRegisterUncreatableType(const char *uri, const char *qmlName)
{
...
inline void initAppInfo(const QString &name)
{
