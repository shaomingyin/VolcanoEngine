...
#define VOLCANO_DRIVERS_GRAPHICS_HEAP_SIZE_ORDER 26
#define VOLCANO_DRIVERS_GRAPHICS_HEAP_SIZE (1 << VOLCANO_DRIVERS_GRAPHICS_HEAP_SIZE_ORDER)
    ...
    m_userMapping(false)
{
    ...
    Q_ASSERT(0 <= offset && offset < VOLCANO_DRIVERS_GRAPHICS_HEAP_SIZE);
    Q_ASSERT(0 < size && size <= VOLCANO_DRIVERS_GRAPHICS_HEAP_SIZE);
}
...
bool Heap::init(void)
{
    ...
    m_heap.allocate(VOLCANO_DRIVERS_GRAPHICS_HEAP_SIZE);
    m_freeSize = VOLCANO_DRIVERS_GRAPHICS_HEAP_SIZE;
...
Buffer *Heap::allocBuffer(int size)
{
    ...
    {
        if (used != nullptr && size > (VOLCANO_DRIVERS_GRAPHICS_HEAP_SIZE - pos))
            return nullptr;
