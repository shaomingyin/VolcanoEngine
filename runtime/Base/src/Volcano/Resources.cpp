...
public:
	NativeFile(const std::filesystem::path& path)
		: fp_(fopen(path.lexically_normal().string().c_str(), "r")) {
		if (fp_ == nullptr) {
...
public:
	...
	bool load(void* buf, int64_t buf_size) override {
		...
		auto read_size = fread(buf, 1, buf_size, fp_);
		return (read_size > 0);
	}
...
Resources::MountInfo::MountInfo(const std::filesystem::path& path, const std::filesystem::path& native_path)
	: path_(path)
...
bool Resources::MountInfo::exists(const std::filesystem::path& path) const {
	bool ret;
	if (std::holds_alternative<std::shared_ptr<mz_zip_archive>>(native_)) {
		auto p = std::get<std::shared_ptr<mz_zip_archive>>(native_);
...
void Resources::MountInfo::enumDir(const std::filesystem::path& path, DirEntrySet& output) const {
	if (std::holds_alternative<std::shared_ptr<mz_zip_archive>>(native_)) {
...
std::shared_ptr<Resource> Resources::MountInfo::get(const std::filesystem::path& path) {
...
void Resources::MountInfo::enumNativeDir(const std::filesystem::path& path, DirEntrySet& output) const {
	auto native_path = std::get<std::filesystem::path>(native_);
...
void Resources::MountInfo::enumZipDir(const std::filesystem::path& path, DirEntrySet& output) const {
...
void Resources::mount(const std::filesystem::path& path, const std::filesystem::path& native_path) {
...
void Resources::umount(const std::filesystem::path& path) {
	std::filesystem::path gpath = normalizedPath(path);
	std::unique_lock<std::mutex> locker(mutex_);
...
bool Resources::exists(const std::filesystem::path& path) const {
	auto gpath = normalizedPath(path);
	if (!isAbsolute(gpath)) {
		throw Error(Errc::InvalidParameter);
	}
...
void Resources::enumDir(const std::filesystem::path& path, DirEntrySet& output) const {
	auto gpath = normalizedPath(path);
	if (!isAbsolute(gpath)) {
		throw Error(Errc::InvalidParameter);
	}
...
std::shared_ptr<Resource> Resources::get(const std::filesystem::path& path) {
	auto gpath = normalizedPath(path);
	if (!isAbsolute(gpath)) {
		throw Error(Errc::InvalidParameter);
	}
