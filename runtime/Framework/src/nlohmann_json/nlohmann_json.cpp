...
namespace nlohmann {
    json loadFromStorage(SDL_Storage* storage, const char* path) {
        ...
        if (size < 1) {
            throw std::runtime_error(fmt::format("Invalid json data {}.", path));
        }
        std::vector<uint8_t> data(size);
        if (!SDL_ReadStorageFile(storage, path, data.data(), size)) {
            throw std::runtime_error(fmt::format("Failed to load json from {}.", path));
        }
    ...
    json findByPath(const nlohmann::json& j, const std::string& json_path) {
        ...
        while (start < json_path.size()) {
            ...
            if (end == std::string_view::npos) {
                throw std::runtime_error(fmt::format("Key {} not found.", json_path));
                break;
            }
