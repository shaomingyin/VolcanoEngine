...
System::System(World::Scene& scene)
	...
	auto view = scene_.view<Transform, JPH::BodyCreationSettings>();
	for (auto ent: view) {
		onBodyAdded(ent);
	}

	scene_.on_construct<Transform>().connect<&System::onBodyAdded>(this);
	scene_.on_construct<JPH::BodyCreationSettings>().connect<&System::onBodyAdded>(this);
}
...
System::~System() {
	...
	scene_.on_construct<Transform>().disconnect<&System::onBodyAdded>(this);
	scene_.on_construct<JPH::BodyCreationSettings>().disconnect<&System::onBodyAdded>(this);
}
...
void System::update(Clock::duration elapsed) noexcept {
	...
	auto view = scene_.view<Transform, JPH::BodyID>();
	for (auto ent: view) {
...
void System::onBodyAdded(entt::entity ent) noexcept {
	...
	auto settings = scene_.try_get<JPH::BodyCreationSettings>(ent);
	if (settings == nullptr) {
		return;
	}
	...
	if (body_id.IsInvalid()) {
		scene_.emplace<JPH::BodyID>(ent, body_id);
	}
}

void System::onBodyRemoved(entt::entity ent) noexcept {
	auto body_id = scene_.get<JPH::BodyID>(ent);
	body_interface_->RemoveBody(body_id);
	scene_.remove<JPH::BodyID>(ent);
}
